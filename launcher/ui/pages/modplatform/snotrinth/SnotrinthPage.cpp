...
#include "SnotrinthPage.h"
#include "ui/dialogs/CustomMessageBox.h"
#include "ui_SnotrinthPage.h"

#include "SnotrinthModel.h"
...
SnotrinthPage::SnotrinthPage(NewInstanceDialog* dialog, QString apiUrl, QWidget* parent)
    : QWidget(parent), ui(new Ui::SnotrinthPage), dialog(dialog), m_fetch_progress(this, false)
{
    ui->setupUi(this);

    ui->searchEdit->installEventFilter(this);
    m_model = new Snotrinth::ModpackListModel(this);
    ...
    connect(&m_search_timer, &QTimer::timeout, this, &SnotrinthPage::triggerSearch);
    ...
    connect(ui->packView->selectionModel(), &QItemSelectionModel::currentChanged, this, &SnotrinthPage::onSelectionChanged);
    connect(ui->versionSelectionBox, &QComboBox::currentTextChanged, this, &SnotrinthPage::onVersionSelectionChanged);
...
SnotrinthPage::~SnotrinthPage()
{
    delete ui;
}

void SnotrinthPage::retranslate()
{
    ui->retranslateUi(this);
}

void SnotrinthPage::openedImpl()
{
    BasePage::openedImpl();
    suggestCurrent();
    triggerSearch();
...
bool SnotrinthPage::eventFilter(QObject* watched, QEvent* event)
{
...
void SnotrinthPage::onSelectionChanged(QModelIndex curr, [[maybe_unused]] QModelIndex prev)
{
    ...
    if (!current.extraInfoLoaded) {
        ...
        auto netJob = new NetJob(QString("Snotrinth::PackInformation(%1)").arg(current.name), APPLICATION->network());
        auto response = std::make_shared<QByteArray>();

        QString id = current.id;

        netJob->addNetAction(Net::ApiDownload::makeByteArray(QString("%1/project/%2").arg(this->m_apiUrl, id), response));
        ...
        QObject::connect(netJob, &NetJob::succeeded, this, [this, response, id, curr] {
            ...
            if (parse_error.error != QJsonParseError::NoError) {
                qWarning() << "Error while parsing JSON response from Snotrinth at " << parse_error.offset
    ...
    if (!current.versionsLoaded) {
        ...
        auto netJob = new NetJob(QString("Snotrinth::PackVersions(%1)").arg(current.name), APPLICATION->network());
        auto response = std::make_shared<QByteArray>();

        QString id = current.id;

        netJob->addNetAction(
            Net::ApiDownload::makeByteArray(QString("%1/project/%2/version").arg(this->m_apiUrl, id), response));
...
void SnotrinthPage::suggestCurrent()
{
    if (!isOpened) {
        return;
    }
...
void SnotrinthPage::triggerSearch()
{
    m_model->searchWithTerm(ui->searchEdit->text(), ui->sortByBox->currentIndex());
    m_fetch_progress.watch(m_model->activeSearchJob().get());
}

void SnotrinthPage::onVersionSelectionChanged(QString version)
{
